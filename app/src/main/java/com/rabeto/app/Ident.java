package com.rabeto.app;import android.content.SharedPreferences;
...
import javax.crypto.spec.SecretKeySpec;/**•هویت کاربر در رابطو.••هر گوشی در اولین اجرا یک «جفت کلید» می‌سازد (کلید خصوصی که هرگز از گوشی بیرون•نمی‌رود + کلید عمومی که آزادانه پخش می‌شود). شناسهٔ کاربر، خودِ اثرانگشتِ•کلید عمومی است. یعنی هیچ‌کس نمی‌تواند شناسهٔ شما را جعل کند: برای جعل باید•کلید خصوصی شما را داشته باشد.•••هر پیام با کلید خصوصیِ فرستنده امضا می‌شود  => فرستنده قابل‌جعل نیست.• private boolean ok = false;
 private PrivateKey priv;
 private PublicKey pub;
 private String pubB64 = "";
 private String id = "";
 private final SecureRandom rnd = new SecureRandom();
 private final Map<String, byte[]> secretCache = new HashMap<String, byte[]>(); public Ident(SharedPreferences prefs) {
     try {
 ...
 public String code() { return codeFor(pubB64); } // ------------------------------------------------------------- fingerprint private static byte[] hash(String pkB64) {
     try {
 ...
 } // ------------------------------------------------------------- signatures public String sign(String data) {
     if (!ok) return "";
     try {
 ...
 } // ------------------------------------------------------------- encryption private byte[] secret(String pkB64) {
